...
class IvaGN(IvaGAlgorithms):
    ...
    def solve(self,X,Winit,Cinit):
        _,_,K = X.shape
        for k in range(K):
            Winit[:, :, k] = torch.linalg.solve(sc.linalg.sqrtm(Winit[:, :, k] @ Winit[:, :, k].T), Winit[:, :, k])
...
class IvaGV(IvaGAlgorithms):
    ...
    def solve(self,X,Winit,Cinit):
        ...
        for k in range(K):
            W_k = Winit[:, :, k]
            # Compute W_k @ W_k^T
            W_k_W_kT = torch.matmul(W_k, W_k.transpose(0, 1))

            # Compute the square root of the matrix using SciPy's sqrtm and convert it to PyTorch tensor
            sqrt_W = torch.tensor(sc.linalg.sqrtm(W_k_W_kT.cpu().numpy()), dtype=Winit.dtype, device=Winit.device)

            # Solve the linear system W_k = sqrt_W^-1 @ X for X
            inter = torch.linalg.solve(sqrt_W, W_k)

            # Update Winit with the solution
            Winit[:, :, k] = inter
...
class TitanIvaG(IvaGAlgorithms):    
    ...
    def __init__(self,color,name='titan_torch',legend='TITAN-IVA-G',alpha=1,nu=0.5,max_iter=20000,max_iter_int=15,max_iter_int_C=1,
                 crit_ext=1e-10,crit_int=1e-10,gamma_w=0.99,gamma_c=1,seed=None):
        super().__init__(name=name,legend=legend,color=color,max_iter=max_iter,crit_ext=crit_ext)
        self.crit_int = crit_int
        self.max_iter_int = max_iter_int
